...
const TestContent: React.FC<TestContentProps> = ({ testId = 'test-content', label = 'Test Content' }) => (
  <Box 
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: 'center' }}>
      <Box sx={{ display: 'flex', gap: 2 }}>
        <button 
          onClick={() => setSize({ width: 400, height: 300 })}
          data-testid="resize-to-large"
        >
          Resize to Large
        </button>
        <button 
          onClick={() => setSize({ width: 200, height: 100 })}
          data-testid="resize-to-small"
        >
          Resize to Small
        </button>
        <button 
...
export const ScreenReaderTest: Story = {
  ...
  args: {
    ...
    children: (
      ...
      >
        <Typography variant="h6" id="panel-title">Panel Title</Typography>
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: 'center' }}>
      ...
      <Resizable
        ...
        width={400}
        height={300}
        data-testid="performance-resizable"
      >
        <Box sx={{ p: 2, bgcolor: 'info.50', height: '100%', overflow: 'auto' }}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: 1 }}>
            {items.map(item => (
              <Paper key={item.id} sx={{ p: 1, textAlign: 'center' }} data-testid={`item-${item.id}`}>
...
const EdgeCasesComponent: React.FC = () => {
  const [scenario, setScenario] = useState<'normal' | 'minimal' | 'maximal' | 'extreme'>('normal');
    ...
    const getProps = () => {
      switch (scenario) {
        case 'minimal':
          return { width: 50, height: 50, minWidth: 50, minHeight: 50, maxWidth: 100, maxHeight: 100 };
        case 'maximal':
          return { width: 800, height: 600, minWidth: 700, minHeight: 500, maxWidth: 900, maxHeight: 700 };
        case 'extreme':
          return { width: 1, height: 1, minWidth: 1, minHeight: 1, maxWidth: 2000, maxHeight: 2000 };
        default:
          return { width: 250, height: 180, minWidth: 100, minHeight: 80, maxWidth: 500, maxHeight: 400 };
      }
    };
    ...
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: 'center' }}>
        ...
        <Resizable
          variant="both"
          {...props}
          data-testid="edge-case-resizable"
        >
          <Box sx={{ 
...
const IntegrationTestComponent: React.FC = () => {
  ...
  const [bottomHeight, setBottomHeight] = useState(100);
    ...
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', height: 400, border: 1, borderColor: 'divider' }}>
