...
import { Box, Typography, Chip, Stack, IconButton, useTheme } from '@mui/material';
    ...
    return (
      <Box width={800}>
        ...
        <Table
          ...
          selectedRows={selectedRows}
          onSelectionChange={setSelectedRows}
          onRowFocus={(event, rowData) =>
            setFocusedRow((rowData as Record<string, unknown>).id as number)
          }
...
export const ThemeVariations: Story = {
  render: function ThemeVariationsTable() {
    ...
    return (
      <Stack spacing={3}>
        <Typography variant="h6">Current theme: {isDark ? 'Dark' : 'Light'} Mode</Typography>
...
export const VisualStates: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const selectedRows = within(selectionTable)
...
export const EdgeCases: Story = {
  render: function EdgeCasesTable() {
    ...
    // Edge case data
    const singleRow = [
      { id: 1, name: 'Only User', email: 'only@example.com', role: 'Admin', status: 'Active' },
    ];
    ...
    const longContentData = [
      {
        ...
        email:
          'very.long.email.address.that.might.cause.overflow@example-domain-with-long-name.com',
    ...
    return (
      <Stack spacing={4}>
        ...
        <Box>
          ...
          <Table data-testid="no-columns-table" data={testData} variant="default" />
        </Box>
      </Stack>
    );
  },
    ...
    return (
      <Box width={900}>
        <Stack direction="row" spacing={2} mb={2}>
          <button onClick={() => setDensity('compact')} data-testid="density-compact">
            Compact
          </button>
          <button onClick={() => setDensity('normal')} data-testid="density-normal">
            Normal
          </button>
          <button onClick={() => setDensity('comfortable')} data-testid="density-comfortable">
            Comfortable
          </button>
        </Stack>
